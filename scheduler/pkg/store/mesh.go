...
// IMPORTANT: This value is synchronized with metrics.MetricAveragingWindow.
// Both read from the same GPU_RELEASE_DELAY_MINUTES environment variable (default: 5 minutes).
// This ensures reserved GPU is held for the same duration as the metric averaging window.
var GPU_RELEASE_DELAY = getGpuReleaseDelay()

// getGpuReleaseDelay reads the GPU release delay from GPU_RELEASE_DELAY_MINUTES environment variable.
// Default is 5 minutes to match the Prometheus metric averaging window.
// This value is shared with metrics.MetricAveragingWindow to ensure consistency.
